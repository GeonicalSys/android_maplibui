...
- `TrackerService`, `WalkEditService` и `ExternalGnssService` подписаны на общий GNSS-only поток
  ...
  запущенного сервиса, а не только после восстановления прежнего трека. Пока
  сервис не активен, пункт остаётся коротким предупреждением; ошибка запуска
  или сохранения точки предупреждает отдельно. Черновик обхода и уже записанные
